...
int tamanhoDaPopulacao = 50;    // Tamanho da população
...
void draw() {
  ...
  for (int i = populacao.size() - 1; i >= 0; i--) {
    ...
    for (Organismo add : adding){
      totalAdd.add(add);
    }
  }

  ArrayList<Organismo> nPopulacao = new ArrayList<>();
